...
def resolve_movement(links):
    ...
    solution = sym.nsolve(
        ...
        [theta2, theta3, x3, y3],
        [
            math.radians(links[2].angle),
            math.radians(links[3].angle),
            links[3].origin[0],
            links[3].origin[1],
        ],
    )
...
def go_to_point(links, x4, y4):
    ...
    e3 = sym.Eq(
        x3, links[1].length * sym.cos(theta1) - links[3].length * sym.cos(theta3)
    )
    e4 = sym.Eq(
        y3, links[1].length * sym.sin(theta1) - links[3].length * sym.sin(theta3)
    )
    e5 = sym.Eq(x2, links[0].length * sym.cos(theta0))
    e6 = sym.Eq(y2, links[0].length * sym.sin(theta0))
    e7 = sym.Eq(
        x4, links[1].length * sym.cos(theta1) + links[4].length * sym.cos(theta3)
    )
    e8 = sym.Eq(
        y4, links[1].length * sym.sin(theta1) + links[4].length * sym.sin(theta3)
    )
    ...
    solution = sym.nsolve(
        ...
        [
            math.radians(links[0].angle),
            math.radians(links[1].angle),
            math.radians(links[2].angle),
            math.radians(links[3].angle),
            links[2].origin[0],
            links[2].origin[1],
