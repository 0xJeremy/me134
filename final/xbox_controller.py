...
class XboxController(threading.Thread):
    ...
    AXISCONTROLMAP = {
        PyGameAxis.LTHUMBX: XboxControls.LTHUMBX,
    ...
    BUTTONCONTROLMAP = {
        PyGameButtons.A: XboxControls.A,
            ...
            for event in pygame.event.get():
                ...
                if event.type == JOYAXISMOTION:
                    ...
                    if event.axis in self.AXISCONTROLMAP:
                        ...
                        yAxis = (
                            True
                            if (
                                event.axis == self.PyGameAxis.LTHUMBY
                                or event.axis == self.PyGameAxis.RTHUMBY
                            )
                            else False
                        )
                        # update the control value
                        self.updateControlValue(
                            self.AXISCONTROLMAP[event.axis],
                    ...
                    if event.axis in self.TRIGGERCONTROLMAP:
                        # update the control value
                        self.updateControlValue(
                            self.TRIGGERCONTROLMAP[event.axis],
                ...
                # button pressed and unpressed
                    ...
                    if event.button in self.BUTTONCONTROLMAP:
                        # update control value
                        self.updateControlValue(
                            self.BUTTONCONTROLMAP[event.button],
